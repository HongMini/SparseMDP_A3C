...
#*******************************************************************************
class Network():
	def __init__(self, scope, num_actions, width, height, channels, gamma, learning_rate, BETA):
		self.scope = scope
	...
	def build_network(self):
		with tf.variable_scope(self.scope):
			...
			self.policy = layer.fully_connected(lstm_out,
												self.num_actions,
												activation_fn=tf.contrib.sparsemax.sparsemax,
	...
	def prepare_loss_function(self):
		with tf.variable_scope(self.scope):
			...
			entropy = tf.reduce_sum(self.policy * 0.5 * (1 - log_policy))
			self.total_loss = 0.5 * value_loss + policy_loss - entropy * self.BETA
